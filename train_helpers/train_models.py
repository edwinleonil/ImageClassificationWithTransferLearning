...
class ModelTrainer:
    def __init__(self, load_model_path, save_model_path, train_data_path, 
        ...
        self.transform = v2.Compose([
            ...
            v2.RandomAffine(degrees=0, scale=(0.8, 1.2)), # add random scale between 0.8 and 1.2
            ...
            v2.Normalize(mean=general_config['mean'], std=general_config['std'])
        ])
    ...
    def validate_epoch(self):
        ...
        return valid_loss / len(self.valid_loader), val_accuracy
    ...
    def train(self):
        ...
        with wandb.init(project=self.project_name, name=self.run_name, 
            ...
            for epoch in range(self.num_epochs):
                avg_train_loss, train_accuracy = self.train_epoch(epoch)
                avg_valid_loss, val_accuracy = self.validate_epoch()
                # Print statistics
                print(f"[Epoch {epoch+1}] Train loss: {avg_train_loss:.4f}, Train-Accuracy: {train_accuracy:.2f} | Validation loss: {avg_valid_loss:.4f}, Val-Accuracy: {val_accuracy:.2f}")
                # log to wandb
                wandb.log({"Epoch": epoch+1, "Train Loss": avg_train_loss, "Train-Accuracy": train_accuracy, "Validation Loss": avg_valid_loss, "Val-Accuracy": val_accuracy})
